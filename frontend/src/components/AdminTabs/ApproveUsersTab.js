...
const ApproveUsersTab = ({ setPingUserList, pingUserList }) => {
  ...
  const handleDenyUser = () => {
    return Promise.all(selectedRows.map((username) => fetchDeny(username)));
  };
  ...
  const fetchDeny = (u) => {
    ...
    return fetch(`http://localhost:8080/api/signup/deny`, opts);
  };
  ...
  return (
    <div id="approveUsersTable" className="approveUsersTable">
      <div style={{ display: "flex", flexDirection: "column" }}>
        ...
        >
          {" "}
          <Block style={{ fontSize: 40, marginRight: 5 }} /> Deny Users{" "}
        </div>
      </div>
      <DataGrid
        loading={loading}
